...
The actionable data list provides checkboxes or radio buttons that enable users to select one or more rows and optionally act on that selection using options in the [toolbar](/components/toolbar/design-guidelines).
...
1. **Expand all:** Toggles expands or collapses every row in the data list at the same time.
