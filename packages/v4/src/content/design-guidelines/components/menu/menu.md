...
A menu component that can be used in a variety of contexts whenever the user needs to choose between multiple values, options, or actions. A menu can be opened in a [dropdown](/components/dropdown), a [select list](/components/select), or it can be revealed upon right clicking on a region within a page.
...
Favoriting or any other optional action can be associated with any menu item. In this case, making an item a favorite will duplicate in at the top of the menu. This may be useful when you have a long list of possible actions and want to “pin” a subset of items to the top of the menu.
...
## Content guidelines
It creating menu item labels, keep in mind the following guidelines:
