...
**Spinners** are used to indicate to users that an action is in progress. For actions that may take a long time use a [progress bar](/components/progress/react) in place of a Spinner.
...
Do not use a spinner to replace a [progress bar](/components/progress/react). Spinners are vague in their promises of results to the user and are best to use when the expected wait time is between 1-5 seconds. Do not use a spinner if the loading process is less than 1 second. For experiences longer than 5 seconds, use a progress bar.
...
**Show a [skeleton](/components/skeleton/design-guidelines) when:**
