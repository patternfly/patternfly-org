...
PatternFly Core is based on the principles of <a href="http://bradfrost.com/blog/post/atomic-web-design/" target="_blank">Atomic design</a> and BEM (Block, Element, Modifier). <a href="http://getbem.com/introduction/" target="_blank">BEM</a> is a popular CSS methodology for building modular, scalable applications. It provides scope, avoids inheritance, and reduces CSS specificity and conflicts.
...
To explore this concept, you can practice overriding the title color custom property in the success variation of the alert component using <a href="https://codesandbox.io/s/html-fundamentals-start-hy2erg" target="_blank">this codesandbox for part 1</a>.
...
In reference to the formula above, it should be: `--pf-c-alert`

#### Step 2.2 
Add the modifier to the custom property name. Reference the <a href="/components/alert/html" target="_blank">alert component documentation</a> on patternfly.org. By scrolling down to the bottom to view the alert component's documentation, locate that the success variation modifier class `pf-m-success` applies to `pf-c-alert`. Add that modifier to the custom property.
...
To explore this concept, override the global link color by setting it to the custom property for danger using <a href="https://codesandbox.io/s/override-global-css-variables-start-llcub8" target="_blank">this codesandbox for part 2</a>. The `index.html` file contains a button and expandable component to demonstrate how changing a global variable has an impact across components.

### Step 1. Familiarize with `index.html`
Note the two button component and the expandable section component in the `<body>` of the `index.html` file.
...
The resulting UI in the codesandbox should match Figure 2. **Note:** The button component and expandable component should inherit different colors.

**Figure 2.**
<img src="../img/global-link-color-red.png" alt="An inline button and a 'show more' expandable section toggle both colored red" width="200" />
...
To explore this concept, add a BEM element and custom styles with CSS custom properties to support a badge element in the label component using <a href="https://codesandbox.io/s/creating-new-component-css-vars-start-t98i06" target="_blank">this codesandbox for part 3</a>. Assume there is a common use case for adding a badge to the left of text in the label component. When adding the custom badge element to the label component, also apply a margin-right of 8px.
...
It should look like this:
`--pf-c-label__badge--MarginRight: var(--pf-global--spacer--sm);`
...
To explore this concept, create a new global custom property for a 5xl font size. Use this new global variation to make a new variation of the title component using <a href="https://codesandbox.io/s/creating-new-global-css-vars-start-cqb8nw" target="_blank">this codesandbox for part 4</a>.
...
It should look like:
`font-size: var(--pf-c-title--m-5xl--FontSize);` inside of `.pf-m-5xl{}`.

Compare your results.
A fully constructed card can be viewed and modified in <a href="https://codesandbox.io/s/creating-new-global-css-vars-final-cugbq0?file=/style.css" target="_blank">this codesandbox solution for part 4</a>. Compare your work with the solution.
