...
|---|---|---|
| `aria-label="[unique text describing the application launcher]"` | `ApplicationLauncher` | Adds an accessible name to the application launcher `<nav>` element and toggle. The value passed in is also used to provide an accessible name to the application launcher menu when open. |
...
| `ariaIsFavoriteLabel="[text describing the item's current favorited status]"` | `ApplicationLauncherItem` | Adds an accessible name to the favorite button when the item is currently favorited. |
| `ariaIsNotFavoriteLabel="[text describing the item's current favorited status]"` | `ApplicationLauncherItem` | Adds an accessible name to the favorite button when the item is not currently favorited. |
...
|---|---|---|
| `aria-label="[unique text describing the application launcher]"` | `.pf-c-app-launcher` | Adds an accessible name to the application launcher `<nav>` element. |
...
| `aria-expanded="false"` | `.pf-c-app-launcher__toggle` | Indicates that the application launcher toggle is collapsed to assistive technologies. **Required** if the toggle is collapsed. |
| `aria-expanded="true"` | `.pf-c-app-launcher__toggle` | Indicates that the application launcher toggle is expanded to assistive technologies. **Required** if the toggle is expanded. |
| `aria-label="[unique text describing the application launcher]"` | `.pf-c-app-launcher__toggle` | Adds an accessible name to the application launcher toggle. |
