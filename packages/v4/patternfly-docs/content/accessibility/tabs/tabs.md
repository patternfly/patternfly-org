...
| `closeButtonAriaLabel="[text describing a tab close button]"` | `Tab` | Adds an accessible name to the close button of a closeable tab when the `onClose` prop is passed into the Tabs sub-component. The value passed in should indicate that the button will close a tab and which tab will be closed. For example, "Close users tab". |
...
| `addButtonAriaLabel="[text describing the tabs add button]"` | `Tabs` | Adds an accessible name to the add button when the `onAdd` prop is also passed in. The value passed in should generally indicate that the button will add a tab to the tablist, and optionally which tablist the tab will be added to. When this prop is not passed in and an add button is rendered, the default `aria-label` on the button will be set to "Add tab". |
| `aria-label="[text describing the tabs component]"` | `Tabs` | Adds an accessible name to the entire set of tabs. Each set of tabs should have a unique accessible name on a page. The value passed in should generally indicate the primary purpose or content of the tablist. |
...
| `leftScrollAriaLabel="[text describing the left scroll button]"` | `Tabs` | Adds an accessible name to the left scroll button when an overflow of tabs occurs. The value passed in should indicate that the button will scroll the tablist left. The default value is "Scroll left". |
| `rightScrollAriaLabel="[text describing the right scroll button]"` | `Tabs` | Adds an accessible name to the right scroll button when an overflow of tabs occurs. The value passed in should indicate that the button will scroll the tablist right. The default value is "Scroll right". |
| `toggleAriaLabel="[text describing the toggle]"` | `Tabs` | Adds an accessible name to the expandable toggle when the `isVertical` prop is also passed in. If the `toggleText` prop is not passed in, this prop must be passed in. |
...
|---|---|---|
| `aria-label="[text describing the tabs component]"` | `.pf-c-tabs` | Adds an accessible name to the entire set of tabs. Each set of tabs should have a unique accessible name on a page. The value passed in should generally indicate the primary purpose or content of the tablist. | 
| `aria-label="[text describing the tabs add button]"` | `.pf-c-tabs__add .pf-c-button.pf-m-plain` | Adds an accessible name to the add button of the tabs component when a tab can by dynamically added. The value passed in should generally indicate that the button will add a tab to the tablist, and optionally which tablist the tab will be added to. |
| `aria-label="[text describing a tab close button]"` | `.pf-c-tabs__item-close .pf-c-button.pf-m-plain` | Adds an accessible name to the close button of a closeable tab. The value passed in should indicate that the button will close a tab and which tab will be closed. For example, "Close users tab". |
| `aria-controls="[id of the tab content controlled by the tab]"` | `.pf-c-tabs__link` | Indicates which tab content panel is controlled by the tab by passing in the ID of the tab content panel as a value. **Required**. |
...
| `aria-expanded="[true or false]"` | `.pf-c-tabs__toggle-button .pf-c-button.pf-m-plain` | Indicates whether an expandable, vertical tabs toggle is expanded (true) or collapsed (false) to assistive technologies. **Required**. |
| `aria-label="[text describing the toggle]"` | `.pf-c-tabs__toggle-button .pf-c-button.pf-m-plain` | Adds an accessible name to the expandable tabs toggle. |
...
| `aria-labelledby="[id of the element that labels the tab content]"` | `.pf-c-tab-content` | Adds an accessible name to the tab content panel. The value passed in should be the ID of the related tab component. Either this or the `aria-labelledby` attribute can be used, but do not use both. |
