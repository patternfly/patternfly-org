...
class Layout extends React.Component {
  ...
  componentDidMount() {
    ...
    if (window.docsearch) {
      ...
      this.setState({
        isBannerOpen: !sessionStorage.getItem('pf4-banner-closed')
      });
    } else {
        ...
        render={data => {
          ...
          const PageToolbar = (
            <Toolbar>
              <ToolbarGroup>
                <ToolbarItem>
                  <Form
                    className="ws-search pf-site-search"
                    onSubmit={event => {
          ...
          return (
            <React.Fragment>
              ...
              >
                {tertiaryNav && <PageSection variant={PageSectionVariants.light}>{tertiaryNav}</PageSection>}
                {this.props.children}
              </Page>
              <Footer />
            </React.Fragment>
