...
Do not try to represent more than six categories in a donut chart. We recommend using the [chart colors](/components/charts/colors) guidelines to represent your data when thresholds are not present.
...
1. **Segment fill:** We recommend using [chart colors](/components/charts/colors) for different items within the donut chart.
